...
class SearchPlaceActivityForPost : AppCompatActivity(), SearchPlaceListAdapter.ItemClickListener,FilterDialogFragment.DialogListener {


    private val viewModel: SearchPlaceViewModel by inject()
    private lateinit var searchPlaceListAdapter: SearchPlaceListAdapter
    private val placeDetailList = mutableListOf<PlaceDetails>()
    private var sort = ""
    private var state = ""
...
//                    searchPlaceListAdapter.setItems(it.data)
                    ...
                    placeDetailList.clear()
                    for (data in it.data){
                        if (placeDetailList.size < it.data.size ){
                            placeDetailList.add(data)
                        }
                    }
                    searchPlaces(search_input.text.toString())
                }
    ...
    override fun onPlaceClick(placeDetial: PlaceDetails) {


        var place = placeDetial
