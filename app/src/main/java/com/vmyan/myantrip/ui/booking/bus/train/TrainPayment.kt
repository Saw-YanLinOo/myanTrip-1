...
import kotlinx.android.synthetic.main.fragment_train_payment.view.*
import www.sanju.zoomrecyclerlayout.ZoomRecyclerLayout

class TrainPayment : Fragment(),BankCardAdapter.ItemClickListener,View.OnClickListener {
    ...
    private var mListener: TrainPayment.OnTrainPayment? = null
    ...
    override fun onCreateView(
        ...
        val view =inflater.inflate(R.layout.fragment_train_payment, container, false)
        view.cardTrainPay.setOnClickListener(this)
        showBankList(view)
        setConfirm()
        return view
    }
    interface OnTrainPayment {
        //void onFragmentInteraction(Uri uri);
        fun onNextPressed(fragment: Fragment?)
    }
    override fun onAttach(context: Context) {
        super.onAttach(context)
        mListener = if (context is OnTrainPayment) {
            context
        } else {
    ...
    fun showBankList(view : View) {
        ...
        view.rv_TrainPayment.layoutManager = layoutManager

        view.rv_TrainPayment.apply {
            setHasFixedSize(true)
            setItemViewCacheSize(20)
        }
        val snapHelper: SnapHelper = LinearSnapHelper()
        snapHelper.attachToRecyclerView(view.rv_hotelpayment)
        view.rv_TrainPayment.isNestedScrollingEnabled = false

        view.rv_TrainPayment.adapter = bankListAdapter
    }
